// 从 memorial-redesign-preview.html 提取 <style>，转译为 in-game 作用域 CSS
// 规则: scope→ "body.tm-phase8-formal .zou-yuan ..."; keyframes→ zmy-*; 丢弃 body/html/*/stage/frame/close/previewbar/toast 根级规则
const fs = require('fs');
const html = fs.readFileSync('memorial-redesign-preview.html','utf8');
...
const DROP=[/^body$/,/^html/,/^\*$/,/^\.zou-stage/,/^\.zou-frame/,/^\.zou-close/,/^\.previewbar/,/^\.toast/];
const kfRename={};
let pass1=[];
// pass1: collect keyframes names
topBlocks(css).forEach(b=>{
  const m=b.match(/^@keyframes\s+([A-Za-z0-9_-]+)/);
  if(m){ kfRename[m[1]]='zmy-'+m[1]; }
});
...
function scopeSel(sel){
  return sel.split(',').map(s=>{
    ...
    if(/^\.zou-yuan\b/.test(s)) return s.replace(/^\.zou-yuan/,'body.tm-phase8-formal .zou-yuan');
    return 'body.tm-phase8-formal .zou-yuan '+s;
  }).filter(Boolean).join(',');
}
function emitNormal(block){
  const {sel,body}=splitRule(block);
  const firstSel=sel.split(',')[0].trim();
  const R='body.tm-phase8-formal .zou-yuan';
  // 特殊根映射
...
+"    st.textContent = '"+escd+"';\n"
+"  }\n";
fs.writeFileSync('_meminstaller.txt', fn);
console.log('rules:', out.length, 'keyframes renamed:', Object.keys(kfRename).join(','));
