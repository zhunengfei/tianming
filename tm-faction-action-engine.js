...
(function(global) {
  ...
  function recordLocalAction(fac, type, payload, rec) {
    var turn = _turn();
    var action = _makeAction('npc_local_' + turn + '_' + _slugId(fac && fac.name), turn, fac, type, Math.floor(Math.random() * 100000), payload || {}, 'local');
  ...
  function applyDecision(fac, decision, opts) {
    ...
    actions.forEach(function(action) {
      ...
      if (res && res.ok) {
        ...
        _recordAction(fac, action, 'skipped', { reason:(res && res.reason) || 'apply failed' });
      }
    });
    if (d && d.rationale) fac._lastLlmRationale = { turn:turn, text:d.rationale };
    ensureStrategy(fac, d, actions);
    return summary;
  }
  ...
  function scoreFactionCandidate(fac, opts) {
    ...
    if (!last) { score += 20; reasons.push('fresh'); }
    else if (turn > last) { score += Math.min(40, (turn - last) * 5); reasons.push('stale'); }
