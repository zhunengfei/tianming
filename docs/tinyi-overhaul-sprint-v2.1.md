# 廷议大改 Sprint·v2.1 (paradigm 重调·v3 亮点集成)

date·2026-05-23·status·**ready for kickoff** (亲读 v3 ~1000 行 verified)
...
**v2.1 hook patch pattern** (学 v3 §11 `_ty3_installDraftHook`)·
...
async function _ty3_v15_appendMinorityRebound(seal, ctx, detail) {
  // 1. v3 effects 已跑·只查·别动 v3 已改的字段
  ...
  for (const npc of minority) {
    ...
    npc.affinity.toEmperor = Math.max(0, (npc.affinity.toEmperor || 50) - finalRebound * 0.6);
  }
  ...
  const martyrCandidates = minority.filter(n => n.dims?.honor >= 0.7 && n.dims?.boldness >= 0.7);
...
3. martyr 入队 (_pendingMartyrEvents)
