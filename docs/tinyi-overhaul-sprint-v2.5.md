# 廷议大改 Sprint·v2.5 (四轮 audit·gate paradigm 修 + chronicleTracker 桥接 verify)

date·2026-05-23·status·**ready for kickoff** (v2.4 + 6 处四轮修订·**含 2 hard paradigm 修正**)
预算·**22.4 - 25.7d** · 关键路径 ~14.5d (vs v2.4 +0.3d·Slice 0 加 0.0a verify v2 + 条件 0.0b 修 v2 dead code)
...
**关键路径** (must-be-sequential)·Slice 0 (+0.0a/0.0b) → 0.5 → 2 → 10a → 2.5 → 3 → 4 → 4.5 → 5 → 6 → 11 = **~14.3-15.2d** (v2.5 +0.3-1.2d·Slice 0 加 v2 verify + 条件修复)·Slice 11 -0.3d 在收口路径·总 ~14.5d 中位
...
| # | 子任务 | 工时 |
|---|---|---|
| 7.5.1 | 6 动作 trigger + state 应用 (loyalty / prestige / health 调) | 0.3d |
| 7.5.2 | 5 ceremony CSS animation (.ty3-cer-flog / strip / dismiss / revoke / reopen) | 0.3d |
| 7.5.3 | 6 pendingEvents 入队 | 0.1d |
| 7.5.4 | smoke·6 case·6 动作 + 5 ceremony 触发 verify | 0.1d |

**DoD** (3 项)·

1. 6 动作触发 + state 写入
2. **(v2.4 新)** 5 联动 ceremony 动画播 (廷杖 5s / 削籍 4s / 摘除 2s / 革职 6s / 更议 3s)·CSS class 按 §5.2.4 表 (本 slice 段)
3. 6 pendingEvents 入队
...
3. **(v2.5 改)** chronicleTracker 桥接 **verify·非 patch**·smoke verify `GM._chronicleTracks` 含 `{type:'chaoyi_pending'}` entry (v3 已调 upsert·非自建 patch)·若 UI 渲染 bug spot 修
