# 廷议大改 Sprint·v2.0 整理版 (final)

date·2026-05-23·status·**ready for kickoff** (亲读 v3 verified)
预算·**22.8 - 25.8d** · 关键路径 ~14d
基础·`tm-tinyi-v3.js` 3942 行·已写完 8 阶段·被 `P.conf.useTinyiV3=false` gate off
...
本 sprint 不是从零做廷议·**是激活已 gate 的 v3 + 增强**·

- **激活**·v3 flag `P.conf.useTinyiV3`·灰度切换 (默认 off·测试期可开)
- **增强 6 件**·
  1. **召集制**·6 资格层 (含 prestige) + 5 后果 + 4 策略 + AI 推荐 27 tag
  2. **民意度 + 言官离心**·政治压力曲线·按 dynasty / daysPerTurn decay
  3. **玩家发言 paradigm 重做**·删 v3 浮按钮·改底部 input + 8 phase 分发 + 13 keyword + 11 intent
  4. **8D dims × persona × mode rule engine**·25 条规则·让 NPC 发言真有立场
  5. **mentor + 召集联动**·门生附议·clientelism mode
  6. **NPC 主动发议题 + 反弹机制完整化**·廷议来源 3 路径·裁决后 loyalty / 党争 真反弹

### Quick Start (实施者第一步)

```
0. 备份·跑 web/scripts/backup-critical-docs.ps1  (D:\tianming-backups\)
1. 读·本 doc §1 (v3 现状) + §4.0 (Slice 0 prep)
...
```
v3 已实现·8 阶段全·UI 全·archon / 用印 / 追责 / 党派 access 都在
v3 未实现·召集制·民意度·言官离心·8D mode 规则·confront chain·玩家灵活发言
v3 bug·    chronicleTracker 桥接断 + L781 typo
本 sprint·上面 6 项 + bug 修
```
...
### 1.11·v3 弹劾结党 spawn 路径 (v3 §15·post-sprint backlog)
...
跟世界系统关联·**Slice 0 启动后必读 v3 §15**·post-sprint 探索完整集成。
...
**关键路径** (must-be-sequential)·Slice 0 → 2 → 2.5 → 3 → 4 → 4.5 → 5 → 6 → 11 = **~14d**
...
| # | 子任务 | 工时 |
|---|---|---|
| 2.5.1 | 6 资格层 (复用 v3 `_ty3_getParties` 等 4 helper) | 0.3d (v1.5 -0.1) |
| 2.5.2 | AI 推荐·27 tag + 三步 + forecast 集成 | 0.5d (v1.5 +0.1) |
| 2.5.3 | 召集 modal·3 视图切换 UI | 0.4d |
| 2.5.4 | 必召 + 漏召 + 5 后果 (v1.4 prestige 加权) | 0.3d |
| 2.5.5 | 朝代差异·明 / 宋 / 唐 三套 JSON | 0.2d |
| 2.5.6 | 民意度·5 档 + dynasty/daysPerTurn decay (v1.4 调小 ±5) | 0.3d |
| 2.5.7 | 言官离心·4 阈值 + 2-3 turn buffer (v1.4 阈值更密) | 0.2d |
| 2.5.8 | mentor + 召集联动·UI 建议同召 mentee | 0.2d |
...
8.  朝代差异·明/宋/唐 三套
9.  GM._convening_民意度·调小后 ±5/次·dynasty 差异 decay·daysPerTurn 归一·5 档影响减半
...
### Slice 4·aiPersonaText + hw/hq + 党派 + learning 注入 (1.5d)

**目标**·v3 phase 2 (`_ty3_phase2_run`) prompt 加 4 段·

```
Section A·persona text 摘要 (200 tokens / NPC)
Section B·8D dims + recognitionState + arc
Section C·当前皇威 X·皇权 Y·档位预测 (huangwei/huangquan)
Section D·党派 stance (focal_disputes + policyStance) + learning (经史学识)
```

**关键** (v1.5.1 新)·**Section C 加 hw/hq**·NPC 看见 hq < 30 → 倾向 confront / martyr。

**LLM cost** (按 §6 预算)·prompt +600 token / NPC·persona 注入率 ≥ 70%。

**DoD**·

1. prompt 体积 +600 token / NPC
2. persona 注入率 ≥ 70%
3. hw/hq 注入·5 case 测·NPC 提及"皇威"/"皇权"概率 ≥ 30%
4. learning 段命中·经史学识 ≥ 70 的 NPC·cite_classic mode 概率 +20%
...
**目标**·v3 phase 2 NPC 发言 mode 系统·6 常朝 mode + 4 廷议 mode。
...
2. mode 分布熵 ≥ 1.8 bit (避塌缩到 1-2 mode)
3. confront mode 5 议题触发率 ≥ 8%
4. clientelism mode 5 议题触发率 ≥ 12%
...
### Slice 7·真 NPC vs NPC confront 链 + 续议 (1.5d)
...
### Slice 8·裁决反弹·minority + 党争 + v3 共存折扣 (1.2d)

**目标**·v3 phase 6 (用印颁行) 之后·v1.4 反弹层·**hook 在 `_ty3_applyArchonGrade` 之后跑·非替换**·

```js
async function _ty3_v15_appendMinorityRebound(decision, opts) {
  // 1. v3 archon effects 已跑 (cohesion / prestige / favor)
  ...
  for (const npc of minority) {
    ...
    const v3Effect    = _ty3_v15_alreadyApplied(npc);
  ...
  if (CY._ty3.conveningPolitics?.tilt === 'oneParty')      rebound *= 1.3;
  if (CY._ty3.conveningPolitics?.tilt === 'fullOneParty')  rebound *= 1.5;
  if (CY._ty3.conveningPolitics?.tilt === 'megaCeremony')  rebound *= 0.8;
  
  // 4. martyr 触发·_pendingMartyrEvents 入队
  
  // 5. 党争·losingParty.tension++·winningParty.morale++
  
  // 6. 民意度 / 言官离心 decay (按 dynasty + daysPerTurn)
}
```
...
1. 5 case 反弹正确
2. minority loyalty / affinity 写入
3. martyr 入队
4. v3 effects + v1.4 rebound 共存·无双重惩罚 (折扣 0.4 验证)
5. conveningPolitics.tilt 二次惩罚正确触发
...
- mentor + 召集联动·UI 在召集 modal 显示 "X 的门生·建议同召 Y/Z"·点 "+一并召门生" 一键加召

**DoD**·

1. 天启 mentor ≥ 30·绍宋 ≥ 15
2. clientelism 5 议题中触发 ≥ 3
3. mentor 联动 UI 工作
4. 一键加召正确·加召的 mentee 不入"漏召"统计
...
**chronicleTracker 桥接 patch** (Slice 11)·
...
if (typeof GM !== 'undefined' && Array.isArray(GM._chronicleTracker)) {
  GM._chronicleTracker.push({
    ...
    turn: GM.turn,
    topic: topic,
    chaoyiTrackId: item.chaoyiTrackId,
    decision: decision,
    grade: grade,
...
#### 5.4.2·6 资格层 (v1.4 加 prestige·亲读 v3 verified)

每个 NPC 通过 `_ty3_calcEligibility(ch, topic, scenario)` 计算 category·6 层叠加·**取最严**·
...
实际取严逻辑·若任一层判"不召"→ 不召·否则·若任一层判"必召"→ 必召·否则取最高 category。
...
const RULES = [
  ...
  // 高 rationality·数据流
  { if: dims.rationality >= 0.7 && tags.includes('finance'), then: 'cite' },
  ...
  // anti-塌缩 guard
  { if: sameModeCount >= 3 && currentMode === 'rebut', then: 'cite' }
];
```
...
const TRAIT_TO_MODE_BIAS = {
  ...
  'trait_jiengong':      { mode: 'cite',         weight: +0.3 },
...
const SMOKE_CASES = [
  // 天启七年
  { scenario: 'tianqi-7-9',   topic: '盐法改革议',     expectedModes: ['lead','rebut','cite'] },
  { scenario: 'tianqi-7-9',   topic: '诛戮魏珰余孽',   expectedModes: ['confront','martyr','clientelism'] },
  // 崇祯元年
  { scenario: 'chongzhen-1',  topic: '九边粮饷',        expectedModes: ['cite','rebut','pivot'] },
  ...
  // 111
  { scenario: '111', topic: '钞法',                      expectedModes: ['cite','rebut'] },
...
**status**·**v2.0·ready for kickoff** · 亲读 v3 verified · 备份机制就绪
**预算**·**22.8 - 25.8d** · 关键路径 ~14d
**预期完成**·2026-06-17 ± 3d
